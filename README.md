...
If not provided explicitly in a LogEntry, Google Cloud Logging library will automatically populate `trace`, `span_id`, and `trace_sampled` fields from detected OpenTelemetry span contexts, or HTTP request headers.
...
If tracing fields are not provided explicitly and no OpenTelemetry context is detected, `trace` / `span_id` fields will be extracted automatically from HTTP headers. 
